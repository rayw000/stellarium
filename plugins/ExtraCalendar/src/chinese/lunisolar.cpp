#include "lunisolar.hpp"

static int lunisolarMonthDays[] =
  {
    ...
    0x1496, 0xb64b, 0xd4a, 0xda8, 0x95b5, 0x56c, 0x12ae, 0x492f, 0x92e, 0xcc96, 0x1a94, 0x1d4a,
    0xada9, 0xb5a, 0x56c, 0x726e, 0x125c, 0xf92d, 0x192a, 0x1a94, 0xdb4a, 0x16aa, 0xad4, 0x955b,
    0x4ba, 0x125a, 0x592b, 0x152a, 0xf695, 0xd94, 0x16aa, 0xaab5, 0x9b4, 0x14b6, 0x6a57, 0xa56,
    0x1152a, 0x1d2a, 0xd54, 0xd5aa, 0x156a, 0x96c, 0x94ae, 0x14ae, 0xa4c, 0x7d26, 0x1b2a, 0xeb55,
    0xad4, 0x12da, 0xa95d, 0x95a, 0x149a, 0x9a4d, 0x1a4a, 0x11aa5, 0x16a8, 0x16d4, 0xd2da, 0x12b6,
    0x936, 0x9497, 0x1496, 0x1564b, 0xd4a, 0xda8, 0xd5b4, 0x156c, 0x12ae, 0xa92f, 0x92e, 0xc96,
    0x6d4a, 0x1d4a, 0x10d65, 0xb58, 0x156c, 0xb26d, 0x125c, 0x192c, 0x9a95, 0x1a94, 0x1b4a,
    0x4b55, 0xad4, 0xf55b, 0x4ba, 0x125a, 0xb92b, 0x152a, 0x1694, 0x96aa, 0x15aa, 0x12ab5, 0x974,
    0x14b6, 0xca57, 0xa56, 0x1526, 0x8e95, 0xd54, 0x15aa, 0x49b5, 0x96c, 0xd4ae, 0x149c, 0x1a4c,
    0xbd26, 0x1aa6, 0xb54, 0x6d6a, 0x12da, 0x1695d, 0x95a, 0x149a, 0xda4b, 0x1a4a, 0x1aa4, 0xbb54,
    0x16b4, 0xada, 0x495b, 0x936, 0xf497, 0x1496, 0x154a, 0xb6a5, 0xda4, 0x15b4, 0x6ab6, 0x126e,
    0x1092f, 0x92e, 0xc96, 0xcd4a, 0x1d4a, 0xd64, 0x956c, 0x155c, 0x125c, 0x792e, 0x192c, 0xfa95,
    0x1a94, 0x1b4a, 0xab55, 0xad4, 0x14da, 0x8a5d, 0xa5a, 0x1152b, 0x152a, 0x1694, 0xd6aa, 0x15aa,
    0xab4, 0x94ba, 0x14b6, 0xa56, 0x7527, 0xd26, 0xee53, 0xd54, 0x15aa, 0xa9b5, 0x96c, 0x14ae,
    0x8a4e, 0x1a4c, 0x11d26, 0x1aa4, 0x1b54, 0xcd6a, 0xada, 0x95c, 0x949d, 0x149a, 0x1a2a, 0x5b25,
    0x1aa4, 0xfb52, 0x16b4, 0xaba, 0xa95b, 0x936, 0x1496, 0x9a4b, 0x154a, 0x136a5, 0xda4, 0x15ac
  };

static int chineseNewYearsInGregorian[] =
  {
    0x75f, 0xec04c, 0xec23f, 0xec435, 0xec649, 0xec83e, 0xeca51, 0xecc46, 0xece3a, 0xed04d,
...
long GregorianToInt(int y, int m, int d) {
  m = (m + 9) % 12;
  y = y - m / 10;
...
Lunisolar getLunisolarFromGregorian(Gregorian g) {
  Lunisolar lunisolar;
  int index = g.year - chineseNewYearsInGregorian[0];
  int data = (g.year << 9) | (g.month << 5) | (g.day);
  int newyear = 0;
  if (chineseNewYearsInGregorian[index] > data) {
    index--;
  }
  newyear = chineseNewYearsInGregorian[index];
  int y = GetBitInt(newyear, 12, 9);
  int m = GetBitInt(newyear, 4, 5);
  int d = GetBitInt(newyear, 5, 0);
  long offset = GregorianToInt(g.year, g.month, g.day) - GregorianToInt(y, m, d);

  int days = lunisolarMonthDays[index];
  int leap = GetBitInt(days, 4, 13);

  int lunarY = index + chineseNewYearsInGregorian[0];
  int lunarM = 1;
  int lunarD;
  offset += 1;
  ...
  lunarD = (int) (offset);
  lunisolar.year = lunarY;
  lunisolar.month = lunarM;
  lunisolar.isleap = false;
  if (leap != 0 && lunarM > leap) {
    lunisolar.month = lunarM - 1;
    if (lunarM == leap + 1) {
      lunisolar.isleap = true;
    }
  }

  lunisolar.day = lunarD;
  return lunisolar;
}
